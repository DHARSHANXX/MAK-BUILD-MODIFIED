...
"@
    ...
    if (Test-Path $tempLog) {
        ...
        if ($m -and $m.Matches) {
            ...
            $sw = $m.Matches[0].Groups[2].Value
            $cw = $m.Matches[0].Groups[3].Value
            $results += "Viewport ${w}px: scrollWidth=$sw, clientWidth=$cw, diff=${diff}px (Passed: $($diff -le 0))"
        } else {
            $results += "Viewport ${w}px: Log matched no pattern"
        }
